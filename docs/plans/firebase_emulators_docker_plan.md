...
| [`firebase.test-unit.json`](../../firebase.test-unit.json) | Emulator ports 8181 / 9160 / **9098** (auth) / 4001 / hub / logging; `0.0.0.0`. Auth is not **9200** (local-dev Storage) or **9099** (full emulator / E2E). |
...
| **emulator** (full) | Served via Firebase Hosting + Functions in emulator | Docker: [`scripts/build-run-on-emulator.sh`](../../scripts/build-run-on-emulator.sh) (`pnpm emulator`) | Requires **built** `packages/web/dist` and `packages/api/dist`. Same **logical** ports as `firebase.json`. **Requires Docker.** Does not share host ports with **local-dev** Compose (local-dev uses [`firebase.local-dev.json`](../../firebase.local-dev.json)). |
...
| Step | Command / action | Expected |
|------|------------------|----------|
| Emulators only | `docker compose -f compose.local-dev.yml up -d` (use the actual filename) | Container healthy; Emulator UI reachable (e.g. `curl -sf -o /dev/null -w "%{http_code}" http://localhost:4002` returns `200` or `302` as applicable). |
| Firestore / Auth ports | `curl -sf http://localhost:8282` (and/or Auth emulator on **9100** if you use a known path) | No connection refused on published ports after a short wait. |
...
4. ~~Update any helper scripts / CI references to the generic Dockerfile.~~ **Done:** scripts use `compose.test-unit.yml`; start uses detached `up -d` (2026-04-08). Helpers: [`scripts/emulator-test-unit-start.sh`](../../scripts/emulator-test-unit-start.sh), [`scripts/emulator-test-unit-stop.sh`](../../scripts/emulator-test-unit-stop.sh), [`scripts/emulator-test-unit-remove.sh`](../../scripts/emulator-test-unit-remove.sh). No in-repo CI workflows reference the old Dockerfile name.
...
| Helper scripts | `./scripts/emulator-test-unit-start.sh` twice | Second run reports already running (if that behavior is kept). `./scripts/emulator-test-unit-stop.sh` stops cleanly. |
...
| [`scripts/emulator-test-unit-start.sh`](../../scripts/emulator-test-unit-start.sh) | `COMPOSE_FILE=compose.test-unit.yml`; detached `up -d`; idempotent when already running. |
| [`scripts/emulator-test-unit-stop.sh`](../../scripts/emulator-test-unit-stop.sh) | Same compose file and service naming as start script. |
...
- [x] [§7 — Documentation](#7-documentation-to-update-master-checklist): all rows reviewed (2026-04-09, Phase F)
...
| **F — Documentation** | Done (2026-04-09) | §7 master checklist applied; drift fixed (`docker-compose.test.yml` → `compose.test-unit.yml`, script names, Compose-first README troubleshooting); [`unit_testing_implementation_plan.md`](unit_testing_implementation_plan.md) and [`unit_testing_sapie.md`](../dev/unit_testing_sapie.md) aligned with current artifacts. |
